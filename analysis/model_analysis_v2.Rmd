...
for(feat in feat_names) {
  ...
  adult_fits[[feat]] = DEoptim(evalFit, lower=.5, upper=5, DEoptim.control(reltol=.001, NP=20, itermax=50), 
...
for(feat in feat_names) {
  feat_df <- get_feature_df(mdat, feat)
  child_fits[[feat]] = DEoptim(evalFit, lower=.5, upper=5, DEoptim.control(reltol=.001, NP=20, itermax=50), 
...
save(mdat, fits, file=here("models/model_data.RData"))
```
...
plot_model_preds <- function(feat_name, fits, hum_data, age="adults") {
  beta = subset(fits, AgeGroup==age & feature==feat_name)$beta
...
ad_supp <- plot_model_preds("support_probability", fits, adult_trial_agg, age="adults")
ch_supp <- plot_model_preds("support_probability", fits, child_trial_agg, age="children")

ggarrange(ad_supp, ch_supp, nrow=1, common.legend = T)
```

```{r}
ad <- plot_model_preds("sharpness_of_support_posjitter_response", fits, adult_trial_agg, age="adults")
ch <- plot_model_preds("sharpness_of_support_posjitter_response", fits, child_trial_agg, age="children")

ggarrange(ad, ch, nrow=1, common.legend = T)
```

```{r}
ad <- plot_model_preds("support_std", fits, adult_trial_agg, age="adults")
ch <- plot_model_preds("support_std", fits, child_trial_agg, age="children")

ggarrange(ad, ch, nrow=1, common.legend = T)
```

```{r}
ad <- plot_model_preds("len_inverse_sharpe_ratio", fits, adult_trial_agg, age="adults")
ch <- plot_model_preds("len_inverse_sharpe_ratio", fits, child_trial_agg, age="children")
