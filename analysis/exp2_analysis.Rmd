...
cd <- rawch %>% filter(Exclude!="Y") %>% 
  dplyr::select(-Version, -Torus0, -Exclude, -Qual1, -Qual2, -Notes, -Note)
...
combo_dat %>% group_by(ID, AgeGroup, relation) %>% # alt, 
  ...
  geom_hline(aes(yintercept=.5), linetype="dashed") + theme_classic()
```
