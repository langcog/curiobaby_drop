...
```{r load-human-data, include=F, echo=FALSE}
...
child_trial_agg <- ch_long %>% group_by(relation, drop, target) %>% # alt, 
  summarize(chose_target=mean(chose_target, na.rm=T), n=n())
...
combo_dat %>% group_by(drop, target, relation, AgeGroup) %>%
  summarise(chose_target = mean(chose_target, na.rm=T)) %>%
  pivot_wider(names_from = AgeGroup, values_from = chose_target) %>%
  ggplot(aes(x=adult, y=child, group=relation, color=relation)) + 
    geom_point(alpha=.7) + theme_bw() + ylim(0,1) + xlim(0,1) + 
