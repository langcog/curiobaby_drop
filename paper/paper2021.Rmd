...
Some features were calculated separately for drop and target objects, such as the inverse SD of each object's final position (*1/SD(Drop Obj Pos)* and *1/SD(Target Pos)*), the mean and maximum final distance of each object from the drop location (e.g., *M(Target Dist)* and *Max(Target Dist)*), and the SD of the normed velocity of each object after first collision (e.g., *Vel(Target)*).
...
The feature with the strongest correlation to adults is Vel(Target) ($r=.33$), but adults' support trials were more correlated with *1/SD(Target Pos)* ($r=.39$), while contain trials were most correlated with *Vel(Drop Obj)* ($r=.54$).
...
These theories are starting to be implemented in deep neural networks, which have recently been found capable of learning forward and inverse physical dynamics (i.e., "intuitive physics") from images when given the ability to "poke" the objects in the scene [@Agrawal2016], and have more recently been used to test which "curious" policies for generating actions result in robust and effective learning of intuitive physics in deep reinforcement learning agents [@Haber2018learning].
