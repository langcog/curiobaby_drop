...
abstract: >
     ...
     AI agents' -- curiosity about objects' physical interactions.
     We let them choose which object to 
     drop another object onto in order to create the most interesting effect. We compared adults' (N=155) and children's choices (N=66; 
     3-7 year-olds) and found that adults show a strong preference for drops that could potentially result in a containment
     relation, a preference that is also shown by children. Adults alone also make choices consistent 
     with achieving support relations. We contextualize our results using heuristic computational models based on 
...
After examining the consistency of people's preferences, we tested the predictive power of a variety of heuristic features derived from simulations of this task using a 3D environment. 
...
```{r fig2-task, fig.env = "figure", fig.pos = "t", fig.align='center', fig.width=3.4, fig.height=3.3, set.cap.width=T, num.cols.cap=1, fig.cap = "Example of a trial given to adults in Experiment 1."}
...
Further, these trial effects were quite reliable across participants. We examined the split-half reliability of adults' choices by repeatedly splitting the data and testing the correlation between the halves.
...
Overall, a few of these features were modestly correlated with people's choices. Table 2 shows the correlation between model preferences (with softmax $\beta = 1$) vs. adults' and children's choice proportions, both to all trials and separately for containment and support trials.
...
Examination of the models with $\beta$ optimized, in contrast, revealed that none  achieved a good fit to human preferences. While the best-fitting feature, *Pr(support)*, captured both adults' and children's overall mean preference for the designed target relations, it explained essentially no trial-level variation and was in fact negatively correlated with choices. This was not due to noisy behavioral data: while the noise ceiling on choice preferences for all trials was $r$=.97 for adults and $r$=.76 for children, the best fitting feature only achieved XX. 
...
<!-- Instead, we found a few features that were partially correlated with people's choices: for example, the inverse standard deviation of the target's final position was the best predictor of both children's and adults' choices on support trials. -->
