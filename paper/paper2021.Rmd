...
abstract: >
     ...
     containment) across development, as well as an adult bias for choosing support relations.
...
We then present a computational theory of how a learner's knowledge state interacts with ongoing perceptual experiences to give rise to curiosity-driven exploratory behavior.
...
ad_supp_cont <- t.test(subset(ad_subj_agg, relation=="contain")$chose_target -
         subset(ad_subj_agg, relation=="support")$chose_target)
```
...
ch_supp_cont <- t.test(subset(ad_subj_agg, relation=="contain")$chose_target -
         subset(ad_subj_agg, relation=="support")$chose_target)
```
...
Averaging each subject's binary responses (1=target relation, 0=alternative) for each trial type revealed a stronger preference for containment relations (M=`r round(ad_rel_means[1,]$M,2)`) than support relations (M=`r round(ad_rel_means[2,]$M,2)`; one-sample `r t_apa(ad_supp_cont, format="rmarkdown", print=F)`).
...
Experiment 1 demonstrated that adults show remarkably consistent preferences for dropping objects on target objects that afford containment relations, and to a lesser extent support relations. 
...
### Participants
Participants were 73 children recruited online via Bing Nursery School and targeted Facebook Ads.
...
After exclusions [based on?], results from `r Nch` were analyzed, including `r age[1,]$N` 3-year-olds, `r age[2,]$N` 4-year-olds, `r age[3,]$N` 5-year-olds, `r age[4,]$N` 6-year-olds, and `r age[5,]$N` 7-year-olds.
...
Based on the binomial distribution, any of the trials on which more than 41 of the 66 participants agree (i.e., >0.64 or <0.36) significantly differ (*p*<.05 [use stricter for mult comparisons? Bonf=.0025]) from chance.
...
As for adults, averaging each child's binary responses for each trial type revealed a stronger preference for containment relations (M=`r round(ch_rel_means[1,]$M,2)`) than support relations (M=`r round(ch_rel_means[2,]$M,2)`; one-sample `r t_apa(ch_supp_cont, format="rmarkdown", print=F)`).
...
Figure 4 shows the proportion of participants' choice of the designed target relation broken down by relation type and age group, showing a preference for the designed relations that grows with age, and that is stronger for containment.
...
Can children's and adults' preferences be predicted by heuristic models of the simulated physical interactions?
Using a physics engine and 3D models of the objects we showed to participants, we constructed a simulation of each trial of the experiment.
For each trial's the two possible drop object choices, we ran 100 simulated drops, and for each drop calculated a variety of features measuring the state of the model after the drop was completed.
