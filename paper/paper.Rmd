...
Below, we give a brief overview of theories of curiosity, relevant developmental empirical findings, and computational models instantiating curiosity in agents that are able to interact with the world in realistic simulated 3D environments.
...
One theory developed to explain the discrepancy between infants' frequent preference for novel stimuli [e.g., @Fantz1964], as well as their occasional preference for familiar stimuli (e.g., faces), is the idea that infants prefer to attend to stimuli that are "moderately discrepant", i.e. that are only somewhat different than the infants' mental representations [@Kinney1976].
Thus, as infants gain experience with various classes of stimuli, they will seek increasingly complex stimuli as their mental representations become more sophisticated [@Dember1957].
Although these theories give us some sense of which situations and knowledge states should engender curiosity, both the mechanism and how actions are selected are not fully specified.
...
Even young infants often exhibit a preference to attend to novel stimuli over more familiar stimuli [e.g., @Fantz1964], and when surprising physical events do occur (e.g., an object appearing to have passed through a wall) it has been shown to stimulate infants' exploration and subsequent learning [@Stahl2015].
...
Many studies of young infants' reasoning about physical objects have focused on occlusion, containment, and covering events, using surprisal (indexed via looking time) as a measure of knowledge [@Baillargeon2007].
...
Here, we investigate toddlers' choices in a more complex
