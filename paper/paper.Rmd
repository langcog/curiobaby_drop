...
First we examine tower height as a function of age, finding that older children tend to build higher towers `r cor_apa(ht_age, format="rmarkdown", print=F)`.

```{r plot-height-by-age, include=F}
...
```{r heatmap-support-by-supported-counts}
...
```{r heatmap-support-by-supported-probs}
tower = subset(tower, tallest_objects!="" & tallest_objects!="pentagon+octahedron, trig prism") # 5 kids who arranged horizontally and 1 kid chose 2 - exclude
df = data.frame(support=NA, supported=NA, n=NA)
for(s in 1:nrow(tower)) {
  objs = unlist(str_split(tower[s,]$tallest_objects, ", "))
