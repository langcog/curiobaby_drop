...
Curiosity may even serve as the impetus for all learning: William James (1899) considered it to be "the impulse towards better cognition." This belief is echoed in Piaget's [-@Piaget1952] theory of development, which holds that children actively construct their knowledge through physical exploration of the world.
From this perspective, children and even infants [@Gopnik2009] can be seen as "little scientists" whose curiosity  selects causal interventions that help them learn about the physical world.
However, there is no integrative, testable theory of the mechanisms and basis of curiosity [@Kidd2015].
More concretely, how does a learner's knowledge state interact with their perception to engender curiosity, and how does curiosity motivate action?
...
First, we need a precise mechanism that specifies how stimuli inspire curiosity. 
Although some early verbal theories of curiosity acknowledge that novel stimuli capture our (and other animals') attention [e.g., @Berlyne1954,@Fantz1964], modern theories hold that learners prefer to attend to stimuli that provide optimal learning opportunities--that are "moderately discrepant" in comparison to our mental representations [@Kinney1976]. Empirical work has verified that infants are indeed sensitive to the level of complexity of stimuli and prefer to attend to stimuli of intermediate complexity [rather than simple or overly-complex; @Kidd2012; @Kidd2014].
Thus, these theories hold that curiosity is inspired by awareness of a knowledge gap [@Loewenstein1994], and that as our mental representations mature our preferences will tend toward more complex stimuli [@Dember1957].
On these accounts, similar to other drives (e.g., hunger), curiosity is sated as relevant information is gained and the knowledge gap is closed. However, these theories have yet to define these perceptual and knowledge-based representations or specify how they are compared.

<!-- The places where curiosity is most manifest tend to be those open-ended environments that are hardest to model--an inviting playground or a surprisingly complex device. -->
<!-- Curiosity-driven behavior comes in a variety of contexts and consists of a wide repertoire of actions.  -->

<!-- While open-ended environments tend to inspire curiosity, choosing an appropriate paradigm, one in which learners both become curious and are able to choose a variety of actions to explore -->


Second, we need finer-grained measures of curiosity-driven behaviors.
What actions do learners select in order to gain information about the properties of objects and laws that govern the physical world? 
...
Behavioral investigations of older children's exploratory play have also used simple attention duration measures to measure curiosity, for example offering children a single complex toy with a number of functions that can be discovered [e.g., @Cook2011; @Bonawitz2012; @Gweon2014], and tallying the number of discovered functions.
...
Further, most environments that inspire curiosity tend to be more open-ended and afford a much wider variety of action choices.

A third desiderata for a testable theory of curiosity is a linking function for evaluating the correspondence between the predictions of our computational theory and the observed patterns of action selection that infants and children show in the same physical task. 
...
Infant studies of surprisal to various physical events involving occlusion, containment, and covering of objects has revealed that even very young infants are unsurprised by many physical interactions [see @Baillargeon2007], but these studies do not specify or predict what physical interventions infants may make in order to learn more about the properties of objects and the laws of physics.

On the computational modeling side, reinforcement learning (RL) researchers have long realized the value of mechanisms that encourage actions that explore the state space in the absence of an apparent reward (which are often sparse), as actions with uncertain outcomes can result in the discovery of new policies with high expected values [@Schmidhuber2010].
Others have built robot models that utilize a form of curiosity to help build predictive and control models of the world [@Oudeyer2007; @Oudeyer2013], although this work pre-trained models on a separate prediction task before implementing curosity-driven learning.
These models' behavior has not yet been directly compared to human behavior in the same task to reveal whether the curiosity driving their behavior mimics people's selection of causal interventions. 

<!-- High-level statement of our approach -->
In this paper we propose a new paradigm that allows us to directly compare children's curiosity about physical interactions with that of an aritifical agent implementing different functional forms of curiosity in a simulated 3D environment, carrying out the same task.
The proposed task, described in detail below, presents learners (children or the agent) with a set of objects from which they can select one to drop in a bin containing a set of target objects, with the potential to create one or more collisions during the drop.
...
First we examine tower height as a function of age. Y/N we find tower height increases with age.
```{r}
