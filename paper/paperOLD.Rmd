...
```{r unique-drop-objects, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3, fig.height=2.5, set.cap.width=T, num.cols.cap=1, fig.cap = "Proportion of unique objects selected by participants. Error bars show bootstrapped 95\\% confidence intervals."}
...
Nsubj_tower = length(unique(hum$SID)) # 51 subjects
...
ht_age = cor.test(hum$age_rounded, hum$height_tallest)
```

The average height of the constructed towers was `r round(mean(na.omit(hum$height_tallest)), 1)` blocks.
We examined tower height as a function of age, and found that older children tend to build higher towers (`r cor_apa(ht_age, format="rmarkdown", print=F)`).

```{r plot-height-by-age, include=F, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3, fig.height=3, set.cap.width=T, num.cols.cap=1, fig.cap = "Tower height by age."}
...
Next we examine the number of times participants supported each object with each other object. 
```{r heatmap-support-by-supported-counts, include=T, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3, fig.height=3, set.cap.width=T, num.cols.cap=1, fig.cap = "Counts of support and supported object relations in participants' towers."}
...
(Could instead show P(supported object | support object).)
```{r heatmap-support-by-supported-probs, include=F, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3, fig.height=3, set.cap.width=T, num.cols.cap=1, fig.cap = "Conditional probabilities of supported object given support object in participants' towers."}
tower = subset(tower, tallest_objects!="" & tallest_objects!="pentagon+octahedron, trig prism") # 5 kids who arranged horizontally and 1 kid chose 2 - exclude
df = data.frame(support=NA, supported=NA, n=NA)
for(s in 1:nrow(tower)) {
  objs = unlist(str_split(tower[s,]$tallest_objects, ", "))
...
<!-- Finally, we look at the distribution of tower heights as a function of the objects chosen as the base of the tower. -->

```{r base-and-top-objects, include=F, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3, fig.height=3, set.cap.width=T, num.cols.cap=1, fig.cap = "Distribution of base and top objects in participants' towers."}
ch_extremes <- hum %>% 
...
rand_drop <- drop_choice_plot(subset(md, policy=="random"), ymax=1, Shapes) + ggtitle("Random")
curio_drop <- drop_choice_plot(subset(md, policy=="curious"), ymax=1, Shapes) + ggtitle("Curious")
noncurio_drop <- drop_choice_plot(subset(md, policy=="noncurious"), ymax=1, Shapes) + ggtitle("Noncurious")
...
```{r combined-model-fig, fig.env = "figure*", fig.pos = "h", fig.align="center", fig.width=7, fig.height=5.5, set.cap.width=T, num.cols.cap=2, fig.cap = "Relative frequency of objects chosen by each model as objects for dropping (first row), and objects hit as targets (second row; pink dots represent hitting empty space), as a function of target/drop set pairing."}

ggarrange(rand_drop, curio_drop, noncurio_drop, 
          rand_targ, curio_targ, noncurio_targ, ncol=3, nrow=2, #labels = c("A", "B", "C"),
          common.legend=T, legend="bottom")
```
